jQuery(document).ready(function($) {
	...
	if (!FlashDetect.installed) {
		...
		soundManager.onready(function() {
			//First screen load
			if(typeof parent.GA_obj != 'undefined'){ parent.GA_obj.trackPageview('/survey/2014/gender/'); }
		...
		$(".surveyGenderOptions li").each(function(){
			$(this).click(function(){
				...
				if(typeof parent.GA_obj != 'undefined'){
 							parent.GA_obj.trackPageview('/survey/2014/age/?gender=' + surveyAnswerGender + '&path=' + parent.location.pathname);
	 			}
				$(".panel-intro").animate({ left: -510 }, 650);//350);
			});
		});
	}
	...
	if($(".surveyAgeOptions").length != 0) {
		...
		$(".surveyAge-custom strong").click(function(){
			...
			if(customAge == "" || customAge == 0) {
				alert("Whoops! You either need to pick an age from 3 through 12, or type in a custom age.");
			} else {
		...
		function advanceToEpilogue() {
			...
			if(typeof parent.GA_obj != 'undefined'){
 							parent.GA_obj.trackPageview('/survey/2014/complete/?' + result);
 						}

			$(".panel-age").animate({ left: -510 }, 650,//350, 
			function(){
				// animation done.
			})
		}
